...
def find_student(records, student_id):
    for index in range(len(records)):
        if records[index]["student_id"] == student_id:
            return index
...
def appeal_score(records):
    ...
    print(
        ...
        f"'{records[index]['name']}' được cộng lại "
        f"{points} điểm."
    )
...
def grade_assignment(records):
    ...
    points = int(
        input("Nhập số điểm gốc đạt được: ")
    )
    ...
    real_points = points * multiplier

    records[index]["current_points"] += real_points

    print(
        f">> Hệ số hiện tại của "
        f"'{records[index]['name']}' là "
        f"x{multiplier}."
    )
