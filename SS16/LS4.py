...
def display_records(records):
    ...
    for index, record in enumerate(records, start=1):
        patient_id, name, birth_year, disease = record.split("-")
...
def add_patient(records):
    ...
    disease = input("Nhập chẩn đoán: ").strip()

    disease = disease.replace("-", " ").capitalize()
...
def update_diagnosis(records):
    ...
    new_disease = (new_disease.replace("-", " ").capitalize())
